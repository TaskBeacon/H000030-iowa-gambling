...
export async function run(root: HTMLElement): Promise<void> {
  ...
  await mountTaskApp({
    root,
    task_id: "H000030-iowa-gambling",
    task_name: "Iowa Gambling Task",
    task_description:
      "HTML preview aligned to local psyflow Iowa Gambling procedure and parameters.",
    settings,
    subInfo,
    stimBank,
              ...
              [
                (_snapshot: TrialSnapshot, runtime: RuntimeView) => {
                  const summary = summarizeBlock(
                    ...
                    blockId,
                    Math.round(controller.total_money)
                  );
                  return stimBank.get_and_format("block_break", {
                    block_num: blockIndex + 1,
                    total_blocks: totalBlocks,
                    current_total: summary.balance_end,
                    block_net_signed: summary.net_sum_signed,
                    adv_rate: summary.adv_rate,
                    mean_rt_ms: summary.mean_rt_ms,
                    timeout_count: summary.timeout_count,
                    deck_a_count: summary.deck_a_count,
                    deck_b_count: summary.deck_b_count,
                    deck_c_count: summary.deck_c_count,
                    deck_d_count: summary.deck_d_count
                  });
                }
              ]
            )
          ...
          [
            (_snapshot: TrialSnapshot, runtime: RuntimeView) => {
              ...
              return stimBank.get_and_format("good_bye", {
                ...
                final_total: summary.balance_end,
